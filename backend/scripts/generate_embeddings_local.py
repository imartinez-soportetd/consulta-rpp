    ...
    async def save_embeddings(self, embeddings_dict: dict) -> int:
        ...
        try:
            for chunk_id, embedding in embeddings_dict.items():
                # Convertir embedding a formato pgvector (array notation)
                # pgvector acepta: [1.0,2.0,3.0] o ARRAY[1.0,2.0,3.0]
                embedding_array = "[" + ",".join(map(str, embedding)) + "]"
                
                # Usar raw SQL con asyncpg para evitar problemas de sintaxis
                await session.execute(
                    text("""
                        UPDATE document_chunks 
                        SET embedding = :embedding_val::vector
                        WHERE id = :chunk_id_val
                    """),
                    {
                        "embedding_val": embedding_array,
